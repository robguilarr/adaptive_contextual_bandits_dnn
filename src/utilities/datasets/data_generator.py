    ...
    def generate_users(self, n):
        ...
        data = {
            ...
            "geo_country": np.random.choice(
                self.countries, size=n, p=self.country_weights
            ),
            "device_os": np.random.choice(self.os_types, size=n, p=self.os_weights),
            "game_day": np.maximum(
                0, np.random.lognormal(mean=3, sigma=1, size=n)
            ).astype(int),
            "distance_avg": np.maximum(
                0, np.random.normal(loc=100, scale=40, size=n)
            ).astype(int),
            "coins_spent": np.maximum(
                0, np.random.normal(loc=1000, scale=600, size=n)
            ).astype(int),
    ...
    def _calculate_ground_truth_probs(self, df):
        ...
        for i, powerup in enumerate(self.powerup_names):
            ...
            if powerup == "sparky_armor":
                ...
                probs[:, i] = np.where(
                    reasons == "laser", probs[:, i] * 5.0, probs[:, i]
                )

            elif powerup == "parachute":
                # Boost if died by wall
                probs[:, i] = np.where(
                    reasons == "wall", probs[:, i] * 5.0, probs[:, i]
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate synthetic data for IAP Bandits."
    )
    parser.add_argument(
        "--train_size", type=int, default=700000, help="Number of training examples"
    )
    parser.add_argument(
        "--val_size", type=int, default=200000, help="Number of validation examples"
    )
    parser.add_argument(
        "--test_size", type=int, default=100000, help="Number of test examples"
    )
    parser.add_argument(
        "--epsilon",
        type=float,
